...
author: | 
 ...
 | *[E-mail](mailto:ajcanepa@ubu.es)* // [LinkedIn](https://www.linkedin.com/in/antonio-canepa-oneto-56b7b633/) // [ResearchGate](https://www.researchgate.net/profile/Antonio-Canepa-Oneto)
...
# library(emo) # (comentado: paquete solo en GitHub y no se usa en el documento) Insert emoticons en el html. Nombres desde https://emoji.muan.co/ // https://github.com/hadley/emo
```
...
Lo que deberemos hacer ahora es escribir un mensaje (parte de él ya estaba escrito) y salir de esa pantalla. Para salir (u otras acciones), abajo de la pantalla tenemos el listado de comandos. el símbolo `^` significa `Ctrl` en windows/linux y `Command` en Mac. Para Windows ver más abajo.
...
En ***Windows*** esto luce un poco diferente y no verás las opciones `^G` ó `^X` (u otras) que se señalan más arriba. Para poder salir de esta pantalla en Windows, una vez inlcuido el mensaje (si no puedes escribir, presiona la tecla "i") deberás de presionar la tecla `esc` (sí la del escape). Verás que en la terminal abajo ya no está la palabra "*-INSERT-*" característica de cuando estás en el modo edición (presionando la letra "i").

Entonces deberás ejecutar `:wq` y presionar `ENTER`.
...
Con este comando saldrás del editor VIM y regresarás a la terminal.
